...
| Модель выдумывает данные | Только подтверждённый контекст, строгий prompt, отдельные структурированные результаты каталога и детерминированный validator перед отправкой |
