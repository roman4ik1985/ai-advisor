...
Перед возвратом успешного ответа backend детерминированно проверяет рискованные утверждения. Цена допускается только при точном совпадении с текущим результатом каталога; утверждение о наличии или обещанный срок доставки заменяются безопасной просьбой уточнить у менеджера, потому что текущий catalog adapter не поставляет live-остатки. Срок гарантии допускается только если он подтверждён найденной knowledge-карточкой. Проверка не меняет успешный JSON-контракт `/api/chat` и не записывает текст диалога в технический validation-log.
