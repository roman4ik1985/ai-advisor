...
Для полного тестового контура одной командой используйте:

```powershell
.\scripts\start-cli-test-browser.ps1
```

Скрипт запускает или переиспользует CLI только на `127.0.0.1:8787`, проверяет
публичную staging-оболочку и открывает в Chrome три вкладки:

- `http://127.0.0.1:8787/` — локальный CLI без расхода OpenAI API credits;
- `https://ai-staging.ledprojector.com.ua/` — доступная тестовому браузеру оболочка;
- `https://ledprojector.com.ua/dev/` — копия магазина, где Basic Auth нужно пройти в браузере один раз.

Если Chrome не установлен, используйте `-Browser Edge` или `-Browser Default`.
Для проверки без открытия вкладок доступен параметр `-NoBrowser`. Скрипт не
останавливает и не перезапускает production API на `8788`.
