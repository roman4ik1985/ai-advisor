# C30 — Telegram customer notification actions and durable outbox acceptance

Date: 2026-08-03
Status: customer-first source revision verified; production remains disabled

## Implemented

- `telegram-order-action-sink.mjs` handles only the private
  `OPEN_NOTIFICATION_SETTINGS` operation for a verified customer binding.
...
- Focused customer menu/action/runtime/release tests: 21/21 PASS.
- Full source suite: 239/239 PASS.
- Tests cover enqueue deduplication, visibility retry, dead-letter boundary,
  Redis failure, action allow-listing, notification persistence and
  enqueue-before-ack runtime behavior.
