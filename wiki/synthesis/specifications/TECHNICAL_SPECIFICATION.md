...
11. Видимая персона выбирается только по server-side allowlisted `operatorId`; клиентский текст не может становиться trusted-инструкцией.
12. `Люми` является оператором по умолчанию и отвечает за подбор; `Спектр` является техническим экспертом по сравнению и объяснению характеристик.
13. Evidence, live-resolvers, validator и ограничения коммерческих утверждений одинаково обязательны для всех операторов.
...
`operatorId` опционален для обратной совместимости. При отсутствии или неизвестном значении backend использует `lumi`; произвольное значение не попадает в trusted prompt. Успешный response contract не расширяется operator metadata и сохраняет прежние ключи.

### 10.3. Публичный реестр операторов

`GET /api/operators` возвращает no-store публичную проекцию: версию контракта, `defaultOperatorId`, имя, цвет и локализованные UI-тексты. Внутренние инструкции, модель, ключи и capabilities в ответ не входят. Endpoint допускает анонимное cross-origin чтение без credentials, так как содержит только публичную конфигурацию виджета.
...
| `src/operator-registry.mjs` | Единый allowlisted реестр, оператор по умолчанию, private instructions и публичная проекция |
